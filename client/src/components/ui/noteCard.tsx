...
function NoteCard({ title, content, subject ,color, fav, createdAt,updatedAt }: { title: string; content: JSONContent;subject:string; color: string;fav:boolean; createdAt:string; updatedAt:string }) {
  ...
  return (
    <div  style={{ backgroundColor: color }} className={`relative text-black p-4 h-78 rounded-xl  w-full max-w-78 mx-auto`}>
      ...
      <div className="bg-[rgb(21,21,21)] rounded-full w-8 h-8 flex justify-center items-center absolute top-4 right-4">
        <FaStar className={`text-lg ${isFav ? 'text-yellow-300' : 'text-white'}`} onClick={() => {
          SetisFav(!isFav)
        }}/>
